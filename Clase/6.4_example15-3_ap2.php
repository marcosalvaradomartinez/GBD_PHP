<?php
	...
	try {
		...
		// Show a result		
		$query = 'SELECT d.department_id, department_name, e.employee_id, first_name, last_name
		FROM departments d, employees e
		WHERE d.department_id = e.department_id 
		ORDER BY d.department_id, employee_id ASC;
		';
		...
		echo "<table border=1>";
			while( null !== ($row = mysqli_fetch_array($table)) ) {  
				echo "<tr>";
				echo "<td>" . $row["department_id"]."</td>"."<td>".$row["department_name"]."</td>"."<td>".$row["employee_id"]."</td>"."<td>".$row["first_name"]."</td>"."<td>".$row["last_name"]."</td>";
				echo "</tr>";
		}
		echo "</table>";
